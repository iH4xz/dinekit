...
function render( $menu_id = 0, $heading = '' ) {
	...
	$config = wp_json_encode(
		array(
			...
			'tableMode'      => (bool) $table_id,
			'tableToken'     => $table_id ? $table_token : '',
			'tableName'      => $table_id ? (string) get_the_title( $table_id ) : '',
			'tablePay'       => ! empty( $settings['table_qr_pay'] ),
			...
			'payNow'         => $pay_live,
			'delivery'       => $table_id ? false : ! empty( $settings['delivery_enabled'] ),
			...
			'i18n'           => array(
				...
				'tableBanner'   => __( 'Ordering at', 'dinekit' ),
				'sendToKitchen' => __( 'Send to kitchen', 'dinekit' ),
				'sentToKitchen' => __( 'Sent to the kitchen!', 'dinekit' ),
				'tableThanks'   => __( 'Your order is on its way. Order more whenever you like — pay at the table when you’re done.', 'dinekit' ),
				'orderMore'     => __( 'Order more', 'dinekit' ),
